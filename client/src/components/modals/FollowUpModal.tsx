...
interface FollowUpModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddFollowUp: (followUpData: any) => void;
}
...
const actionTypes: ActionType[] = [
  ...
  {
    id: 'redirect-agent',
    name: 'Redirect to Agent',
    description: 'Transfer conversation to another agent',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M7.50082 6C7.50082 3.51472 9.51554 1.5 12.0008 1.5C14.4861 1.5 16.5008 3.51472 16.5008 6C16.5008 8.48528 14.4861 10.5 12.0008 10.5C9.51554 10.5 7.50082 8.48528 7.50082 6Z" fill="#525866"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M3.75207 20.1053C3.82941 15.6156 7.4928 12 12.0008 12C16.5089 12 20.1724 15.6157 20.2496 20.1056C20.2547 20.4034 20.0832 20.676 19.8125 20.8002C17.4335 21.8918 14.7873 22.5 12.0011 22.5C9.21468 22.5 6.56825 21.8917 4.18914 20.7999C3.91847 20.6757 3.74694 20.4031 3.75207 20.1053Z" fill="#525866"/>
      </svg>
    ),
    color: '#525866',
    requiresConfig: true
  },
  {
    ...
    description: 'Automatically pass to next agent',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  ...
  },
  {
    id: 'create-ticket',
    name: 'Create Ticket',
    description: 'Generate a support ticket instantly',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M5.85121 3.50019C6.12746 3.19155 6.10119 2.7174 5.79255 2.44116C5.4839 2.16491 5.00975 2.19118 4.73351 2.49982C3.53256 3.84163 2.69691 5.51948 2.38546 7.37592C2.31692 7.78442 2.59252 8.17114 3.00103 8.23967C3.40953 8.3082 3.79625 8.0326 3.86478 7.6241C4.12789 6.05581 4.83387 4.63686 5.85121 3.50019Z" fill="#525866"/>
        <path d="M19.2678 2.49982C18.9916 2.19118 18.5175 2.16491 18.2088 2.44116C17.9002 2.7174 17.8739 3.19155 18.1501 3.50019C19.1675 4.63686 19.8735 6.05581 20.1366 7.6241C20.2051 8.0326 20.5918 8.3082 21.0003 8.23967C21.4088 8.17114 21.6844 7.78442 21.6159 7.37592C21.3045 5.51948 20.4688 3.84163 19.2678 2.49982Z" fill="#525866"/>
  ...
  },
  {
    ...
    description: 'Set a future follow up reminder',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
export default function FollowUpModal({ isOpen, onClose, onAddFollowUp }: FollowUpModalProps) {
  const [selectedAction, setSelectedAction] = useState<string>('');
  const [actionName, setActionName] = useState<string>('');
  const [webhookUrl, setWebhookUrl] = useState<string>('');
  const [delay, setDelay] = useState<string>('30 minutes');

  // Auto-select first action when modal opens (if needed)
  React.useEffect(() => {
  ...
  const selectedActionData = actionTypes.find(action => action.id === selectedAction);

  const handleSubmit = () => {
    if (!selectedAction || !actionName) return;
    ...
    const followUpData = {
      ...
      webhookUrl: selectedAction === 'webhook' ? webhookUrl : undefined,
      delay,
      ...(selectedActionData ? {
        description: selectedActionData.description,
        icon: selectedActionData.icon,
        color: selectedActionData.color,
      } : {})
    };
      ...
      <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl p-[24px] w-full max-w-[500px] mx-4">
        ...
        <div className="flex items-center justify-center mb-6">
          <div className='flex flex-col items-center justify-center'>
            <h2 className="text-[20px] font-normal text-gray-900 dark:text-white">Add Follow-Up Action to your Agent</h2>
            <p className="text-[16px] text-gray-500 dark:text-gray-400 mt-1">Select the action type</p>
          </div>
        </div>

        {/* Action Type Selection */}
        <div className="mb-6">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Select action type</h3>
          <div className="grid grid-cols-2 gap-3">
            {actionTypes.map((action) => (
              ...
              >
                <div className={`p-3 rounded-lg h-full flex flex-row gap-[16px] items-center dark:bg-[#1A1A1A] ${
                  selectedAction === action.id 
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-medium text-[16px] text-[${action.color}]`}>
                      {action.name}
                    </span>
          ...
          <div className="mb-6 p-4 bg-white dark:bg-[#1A1A1A] border border-[#D5D5D5] dark:border-[#333] rounded-lg">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                {selectedActionData?.name} Configuration
              </span>
            </div>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder={`My ${selectedActionData?.name} Action`}
            ...
            {/* Webhook URL (only for webhook action) */}
            ...
            <div className="mb-4">
              ...
              <select
                ...
                className="bg-[#F6F6F6] dark:bg-[#333] w-full p-[14px_16px] rounded-[6px] text-[14px] focus:outline-none focus:ring-2 focus:ring-[#DA46F8] focus:border-transparent font-inter text-black dark:text-white border dark:border-gray-600"
              >
        ...
        <div className="flex gap-3 pt-4 justify-end">
          ...
          <button
            ...
            disabled={!selectedAction || !actionName}
