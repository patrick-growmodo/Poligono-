...
export default function Sidebar({ user }: UserProps) {
  ...
  return (
    <div className={`h-[100vh] sticky top-0  pt-[20px] pb-[16px] bg-white dark:bg-[#1A1A1A]  flex flex-col border_custom  transition-all duration-300 ${isMinimized ? 'w-[84px] px-[20px]' : 'w-[272px] min-w-[272px] px-[20px]'}`}>
      ...
      <div className={` ${isMinimized ? 'p-[0px]' : 'p-[12px]'}`}>
        <div className={`flex items-center gap-[16px] ${isMinimized ? 'flex-col' : ''}`}>
          <div className="relative">
            <Image 
              src={user?.picture || '/images/gallery/profile-1.png'} 
              alt="User"
              width={30} 
              height={30}
          ...
          <div className="flex-1">
            <p className="text-[16px] truncate max-w-[100px] font-medium text-gray-900 dark:text-white">{user?.name}</p>
            <p className="text-[12px] text-[#525866] dark:text-gray-400">Free plan</p>
          </div>
          )}
          {!isMinimized && (
            <a href="/auth/logout" className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
