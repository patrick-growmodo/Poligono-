...
export default function CreateAgent() {
  ...
  const handleRagFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(event.target.files || []);
    setRagFiles(prev => [...prev, ...files]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/agents-list/create', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setToast({
          message: 'Agent created successfully! Redirecting to agents page...',
          type: 'success',
          isVisible: true
        });
        console.log('✅ Toast state set, will redirect in 2 seconds...');
        
        // Redirect after 2 seconds to allow toast to be seen
        setTimeout(() => {
        ...
        setToast({
          message: data.error || data.message || 'Failed to create agent. Please try again.',
          type: 'error',
          isVisible: true
        });
      }
       ...
       setToast({
         message: 'An error occurred while creating the agent. Please try again.',
         type: 'error',
         isVisible: true
       });
     } finally {
  ...
  const isFormValid = () => {
    return agentName.trim() !== '' && description.trim() !== '';
  };
                  ...
                  <React.Fragment key={step.id}>
                    <div className="flex items-center relative">
                      <div className="flex items-center">
                        ...
                        >
                          {step.id < currentStep ? '✓' : <Image src={step.icon} alt={step.name} width={12} height={12} className="dark:brightness-150" />}
                        </div>
                        <span
  ...
  <path fillRule="evenodd" clipRule="evenodd" d="M8.14645 2.14645C8.34171 1.95118 8.65829 1.95118 8.85355 2.14645L11.8536 5.14645C12.0488 5.34171 12.0488 5.65829 11.8536 5.85355C11.6583 6.04882 11.3417 6.04882 11.1464 5.85355L9 3.70711L9 11.5C9 11.7761 8.77614 12 8.5 12C8.22386 12 8 11.7761 8 11.5L8 3.70711L5.85355 5.85355C5.65829 6.04882 5.34171 6.04882 5.14645 5.85355C4.95118 5.65829 4.95118 5.34171 5.14645 5.14645L8.14645 2.14645ZM2.5 11C2.77614 11 3 11.2239 3 11.5V13C3 13.5523 3.44772 14 4 14H13C13.5523 14 14 13.5523 14 13V11.5C14 11.2239 14.2239 11 14.5 11C14.7761 11 15 11.2239 15 11.5V13C15 14.1046 14.1046 15 13 15H4C2.89543 15 2 14.1046 2 13V11.5C2 11.2239 2.22386 11 2.5 11Z" fill="#0F172A"/>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div>
                      ...
                      />
                      <label
                        htmlFor="rag-files-upload"
                        className="flex items-center gap-2 px-4 py-2 border border-[#0F172A] dark:border-gray-600 rounded-full bg-white dark:bg-[#2A2A2A] text-[14px] font-medium text-[#0F172A] dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#333] cursor-pointer"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                          <path fillRule="evenodd" clipRule="evenodd" d="M8.14645 2.14645C8.34171 1.95118 8.65829 1.95118 8.85355 2.14645L11.8536 5.14645C12.0488 5.34171 12.0488 5.65829 11.8536 5.85355C11.6583 6.04882 11.3417 6.04882 11.1464 5.85355L9 3.70711L9 11.5C9 11.7761 8.77614 12 8.5 12C8.22386 12 8 11.7761 8 11.5L8 3.70711L5.85355 5.85355C5.65829 6.04882 5.34171 6.04882 5.14645 5.85355C4.95118 5.65829 4.95118 5.34171 5.14645 5.14645L8.14645 2.14645ZM2.5 11C2.77614 11 3 11.2239 3 11.5V13C3 13.5523 3.44772 14 4 14H13C13.5523 14 14 13.5523 14 13V11.5C14 11.2239 14.2239 11 14.5 11C14.7761 11 15 11.2239 15 11.5V13C15 14.1046 14.1046 15 13 15H4C2.89543 15 2 14.1046 2 13V11.5C2 11.2239 2.22386 11 2.5 11Z" fill="#0F172A"/>
                        </svg>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {ragFiles.length === 0 ? (
                      <div className="col-span-2 text-center py-8 text-gray-500 dark:text-gray-400">
                        No files uploaded yet. Click "Upload files" to add RAG documents.
                      </div>
                    ) : (
                      ragFiles.map((file, index) => (
                        <div key={index} className="border border-gray-200 dark:border-gray-600 rounded-[12px] p-[12px_10px_12px_16px] bg-white dark:bg-[#2A2A2A]">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[14px] font-medium truncate max-w-[100px] text-gray-900 dark:text-white">{file.name}</span>
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="none">
                                <path fillRule="evenodd" clipRule="evenodd" d="M1.5 8.5C1.5 4.91015 4.41015 2 8 2C11.5899 2 14.5 4.91015 14.5 8.5C14.5 12.0899 11.5899 15 8 15C4.41015 15 1.5 12.0899 1.5 8.5ZM10.4069 7.29062C10.5674 7.06591 10.5153 6.75364 10.2906 6.59313C10.0659 6.43263 9.75364 6.48467 9.59313 6.70938L7.4362 9.72909L6.35355 8.64645C6.15829 8.45118 5.84171 8.45118 5.64645 8.64645C5.45118 8.84171 5.45118 9.15829 5.64645 9.35355L7.14645 10.8536C7.25037 10.9575 7.39476 11.0104 7.54124 10.9983C7.68772 10.9862 7.82144 10.9102 7.90687 10.7906L10.4069 7.29062Z" fill="#43D77E"/>
                              </svg>
                            </div>
                            <button 
                              className="p-1 hover:bg-gray-100 dark:hover:bg-[#333] rounded"
                              onClick={() => setRagFiles(prev => prev.filter((_, i) => i !== index))}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <span className="px-2 py-1 bg-[#DBFCE7] text-[#0B944A] text-[12px] font-medium border  border-[#0B944A] dark:border-[#0B944A] rounded-full">
                              processed
                            </span>
                            <span className="text-[12px] text-[#525866] dark:text-gray-300">
                              {file.type === 'application/pdf' ? 'PDF' : file.type} • {(file.size / 1024 / 1024).toFixed(2)} MB
                            </span>
                          </div>
                        </div>
                      ))
              ...
              <div className="space-y-6">
                ...
                <div className="border border-gray-200 dark:border-gray-600 rounded-[8px] bg-white dark:bg-[#2A2A2A] p-[24px]">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-[18px] font-normal text-gray-900 dark:text-white">
                        {agentName || 'Customer Support Agent'}
                      </h3>
                      <span className="p-[7px_12px] bg-gray-100 dark:bg-[#3A3A3A] text-gray-700 dark:text-gray-300 text-[12px] font-medium rounded-full">
                        Draft
                      </span>
                    </div>
                  </div>
                         {/* System Prompt Section */}
                         <hr className="border-gray-200 dark:border-gray-600 mb-[24px]" />

                <div className="mb-[24px]">
                  <h4 className="text-[16px] font-normal text-gray-900 dark:text-white mb-3">
                    System Prompt
                  </h4>
                  <div className="border border-gray-200 dark:border-gray-600 rounded-[8px] p-4 bg-[#F6F6F6] dark:bg-[#2A2A2A]">
                    <p className="text-[14px] text-gray-700 dark:text-gray-300 leading-relaxed">
                      {systemPrompt || "You are a highly capable and reliable AI assistant designed to help users complete tasks efficiently. Always interpret their intent accurately, ask clarifying questions when needed, and respond in a clear, structured, and friendly tone."}
                    </p>
                  </div>
                </div> 
                ...
                <div>
                  <h4 className="text-[16px] font-normal text-gray-900 dark:text-white mb-3">
                    Rag Files ({ragFiles.length})
                  </h4>
                  {ragFiles.length === 0 ? (
                    <div className="text-center py-4 text-gray-500 dark:text-gray-400">
                      No RAG files uploaded
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {ragFiles.map((file, index) => (
                        <div key={index} className="border border-gray-200 dark:border-gray-600 rounded-[8px] p-4 bg-white dark:bg-[#2A2A2A]">
                          <div className="flex items-center gap-2 mb-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                              <path d="M9 1H3C2.44772 1 2 1.44772 2 2V14C2 14.5523 2.44772 15 3 15H13C13.5523 15 14 14.5523 14 14V6L9 1Z" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                              <path d="M9 1V6H14" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                            <span className="text-[14px] truncate max-w-[100px] font-medium text-gray-900 dark:text-white">{file.name}</span>
          ...
          <div className="flex items-center justify-between mt-[24px] pt-6">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M7 13L2 8M2 8L7 3M2 8L14 8" stroke="#525866" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg> Back
              </button>

              <button
                type="submit"
                disabled={isSubmitting || (currentStep === steps.length && !isFormValid())}
                style={{
                  background: 'linear-gradient(90deg, #6940E4 0.07%, #FF5AFE 124.81%), #E8E8E8'
                }}
                className="flex items-center  p-[8px_32px] gap-[10px] text-white text-[14px] font-medium rounded-[6px]
                  hover:bg-[#553C9A] disabled:opacity-50 disabled:cursor-not-allowed
                  bg-gradient-to-r from-[#6940E4] to-[#FF5AFE]
                  "
              >
                {isSubmitting ? (
                  <>
                  ...
                  <>
                    ...
                    {currentStep === steps.length ?
                      ...
                      :
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path d="M9 3L14 8M14 8L9 13M14 8H2" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                      }
                  </>
                )}
